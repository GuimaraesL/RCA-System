...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Improved form detection: avoid matching "Card" or "Credit" in text/component names
        has_form = bool(re.search(r'<form|<input\b|<select\b|<textarea\b|type=["\']password["\']', content, re.IGNORECASE))
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'playwright-report', 'test-results', 'coverage'}]
            for file in files:
