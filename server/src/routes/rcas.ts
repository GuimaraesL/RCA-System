...
// POST /api/rcas/bulk-delete - Exclusão em massa
router.post('/bulk-delete', (req: Request, res: Response) => {
    try {
        ...
        console.log(`🗑️ Bulk Deleting ${ids.length} RCAs...`);

        db.exec('BEGIN TRANSACTION');
        const stmt = db.prepare('DELETE FROM rcas WHERE id = ?');
        ...
        console.error('Erro no bulk delete de RCAs:', error);
