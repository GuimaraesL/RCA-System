...
export const ShortcutsHelpModal: React.FC<ShortcutsHelpModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        ...
        >
            <div
                className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700 animate-scale-in"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Cabeçalho */}
                <div className="px-8 py-6 bg-gradient-to-r from-primary-50 to-indigo-50 dark:from-slate-700 dark:to-slate-800 flex items-center gap-4 border-b border-slate-100 dark:border-slate-700">
                    <div className="p-2.5 rounded-xl bg-white dark:bg-slate-800 shadow-sm text-primary-500">
                ...
                <div className="p-6 space-y-5 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {SHORTCUT_GROUPS.map((group, gIdx) => (
                        <div key={gIdx}>
                            {gIdx > 0 && (
                                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 mt-1">
                                    {t(group.titleKey)}
                                </p>
                            )}
                            <div className="space-y-1">
                                {group.items.map((shortcut, idx) => (
                                    <div
                                        key={idx}
                                        className="flex items-center gap-4 px-4 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                                    >
                                        <shortcut.icon
                                            size={16}
                                        ...
                                        <div className="flex gap-1.5">
                                            {shortcut.keys.map((key) => (
                                                <kbd
                                                    key={key}
                                                    className="px-2.5 py-1 text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg shadow-sm font-mono"
                                                >
                                                    {key}
                                                </kbd>
                                            ))}
