  ...
  return (
    <div className="select-none">
      ...
      >
        ...
        <span className={`text-xs ${isSelected ? 'font-black' : isSelectable ? 'font-bold' : 'font-black text-slate-700 dark:text-slate-400 opacity-80'} truncate`}>{node.name}</span>
...
export const AssetSelector: React.FC<AssetSelectorProps> = ({ assets, onSelect, selectedAssetId, selectableTypes }) => {
  ...
  if (!sortedAssets || sortedAssets.length === 0) {
    return (
      <div className="p-12 text-center bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
  ...
  return (
    <div className="border border-slate-200/60 dark:border-slate-800 rounded-2xl p-3 bg-white dark:bg-slate-900 shadow-inner h-72 overflow-y-auto custom-scrollbar flex flex-col gap-1.5">
