...
export const MandatoryFieldSelector: React.FC<MandatoryFieldSelectorProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-700 flex flex-col h-full overflow-hidden transition-all duration-500 hover:shadow-xl hover:shadow-primary-500/5 hover:border-primary-200 dark:hover:border-primary-800 group">
                    ...
                    return (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 group/btn ${isSelected
                                ? 'bg-white dark:bg-slate-800 border-primary-500 text-primary-700 dark:text-primary-400 shadow-md ring-4 ring-primary-500/5'
            ...
            <div className="px-8 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                ...
                {selectedFields.length > 0 && (
                    <button
                        type="button"
                        onClick={() => onChange([])}
                        className="text-[10px] font-black text-primary-600 hover:text-rose-600 uppercase tracking-widest transition-colors"
                    >
