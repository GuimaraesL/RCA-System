...
import { isComponentVisible } from '@/services/TemplateService';
...
const Step4InvestigationComponent: React.FC<Step4Props> = ({ data, onChange, taxonomy, showHra, isFieldRequired, errors }) => {
    ...
    const ishikawaCategories = [
        { key: 'method', label: t('wizard.step4.ishikawaCategories.method'), color: 'bg-primary-100 text-primary-800 border-primary-200' },
    ...
    // Component visibility based on template
    const showFiveWhys = isComponentVisible(data.analysis_type, 'FIVE_WHYS');
    const showIshikawa = isComponentVisible(data.analysis_type, 'ISHIKAWA');
    const showRootCauses = isComponentVisible(data.analysis_type, 'ROOT_CAUSES');

    return (
        <div className="space-y-8 max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{t('wizard.step4.title')}</h2>
                    <p className="text-slate-600 dark:text-slate-400">{t('wizard.step4.subtitle')}</p>
                </div>
            </div>
            ...
            {showFiveWhys && (
                <div
                    ...
                    className={`p-6 rounded-xl border shadow-sm transition-all ${errors?.five_whys ? 'border-red-500 ring-2 ring-red-50 dark:ring-red-900/20' : ''} ${useAdvancedMode ? 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800' : 'bg-gradient-to-br from-primary-50 to-indigo-50 dark:from-primary-950/20 dark:to-indigo-950/20 border-primary-200 dark:border-primary-800'}`}
                >
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-xl font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                {t('wizard.step4.fiveWhysTitle')} {isFieldRequired('five_whys') && <span className="text-red-500">*</span>}
                                {useAdvancedMode && <span className="bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-xs px-2 py-0.5 rounded-full border border-primary-200 dark:border-primary-800">{t('wizard.step4.advancedMode')}</span>}
                            </h3>
                        ...
                        <div className="flex gap-2">
                            {!useAdvancedMode && (
                                <Button variant="ghost" onClick={() => setUseAdvancedMode(true)} className="text-primary-600 dark:text-primary-400 hover:bg-primary-100 dark:hover:bg-primary-900/30">
                    ...
                    {useAdvancedMode ? (
                        ...
                        <div className="space-y-4">
                            {(data.five_whys || []).length === 0 && (
                                <div className="text-center p-8 border-2 border-dashed border-primary-200 dark:border-primary-800 rounded-lg bg-primary-50/50 dark:bg-primary-900/10">
                                    <p className="text-primary-800 dark:text-primary-200 font-medium mb-2">{t('wizard.step4.addWhy')}</p>
                                    <Button
                            ...
                            {(data.five_whys || []).map((w, index) => (
                                <div key={index} className="flex items-start gap-3 group">
                                    <div className="flex-shrink-0 w-8 h-8 bg-primary-600 dark:bg-primary-500 text-white rounded-full flex items-center justify-center font-bold mt-2 shadow-sm">
                                        {index + 1}
                                    </div>
                                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-semibold text-primary-700 dark:text-primary-300 mb-1 uppercase">{t('wizard.step4.fiveWhys.whyEffect')}</label>
                                            <Input
                                        ...
                                        <div>
                                            <label className="block text-xs font-semibold text-primary-700 dark:text-primary-300 mb-1 uppercase">{t('wizard.step4.fiveWhys.whyCause')}</label>
                                            <Input
                                    ...
                                    <button
                                        ...
                                        className="mt-8 p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors opacity-0 group-hover:opacity-100"
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            ...
                            {(data.five_whys || []).length > 0 && (
                                <div className="flex justify-between items-center mt-4">
                                    <div className="p-2 bg-white/60 dark:bg-slate-800/60 rounded-lg border border-primary-300 dark:border-primary-700">
                                    ...
                                    <Button
                                        ...
                                        variant="secondary"
                                        size="sm"
                                        className="bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-800"
                                        data-testid="btn-add-why"
                                    >
            ...
            {showIshikawa && (
                ...
                >
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{t('wizard.step4.ishikawaTitle')} {isFieldRequired('ishikawa') && <span className="text-red-500">*</span>}</h3>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {ishikawaCategories.map((category) => (
                            <div key={category.key} className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            ...
            {showRootCauses && (
                <div
                    ...
                    className={`p-6 rounded-xl border-2 shadow-sm transition-all ${errors?.root_causes ? 'border-red-500 ring-2 ring-red-50 dark:ring-red-900/20' : 'border-yellow-300 dark:border-yellow-700'} ${canDefineRootCause ? 'bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-950/20 dark:to-amber-950/20' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700'}`}
                >
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{t('wizard.step4.rootCausesTitle')} {isFieldRequired('root_causes') && <span className="text-red-500">*</span>}</h3>
                        {canDefineRootCause && (
                            <button
                    ...
                    <div className="space-y-4">
                        {(data.root_causes || []).length === 0 && canDefineRootCause && (
                            <p className="text-sm text-slate-500 dark:text-slate-400 italic text-center p-4 bg-white/50 dark:bg-slate-900/50 rounded-lg border border-dashed border-yellow-200 dark:border-yellow-800">
                        ...
                        {(data.root_causes || []).map((rc, idx) => (
                            <div key={rc.id} className={`grid grid-cols-12 gap-4 items-start bg-white dark:bg-slate-800 p-4 rounded-lg border shadow-sm group ${(!rc.root_cause_m_id || !rc.cause.trim()) && errors?.root_causes ? 'border-red-300 ring-1 ring-red-50 dark:ring-red-900/20' : 'border-yellow-200 dark:border-yellow-800'}`}>
                                ...
                                <div className="col-span-12 md:col-span-1 flex justify-center pt-8">
                                    <button onClick={() => removeRootCause(idx)} className="text-slate-300 hover:text-red-500 transition-colors p-2 hover:bg-red-50 rounded-full">
                                        <Trash2 size={18} />
                                    </button>
                                </div>
                            </div>
                    ...
                    {(showHra || isComponentVisible(data.analysis_type, 'HRA')) && (
