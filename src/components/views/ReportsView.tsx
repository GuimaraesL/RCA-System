    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case ACTION_STATUS_IDS.APPROVED:
                return (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-black uppercase tracking-wider border border-emerald-100 dark:border-emerald-900/30 shadow-sm">
            ...
            case ACTION_STATUS_IDS.IN_PROGRESS:
                return (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-[10px] font-black uppercase tracking-wider border border-amber-100 dark:border-amber-900/30 shadow-sm">
            ...
            case ACTION_STATUS_IDS.VERIFIED:
                return (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 text-[10px] font-black uppercase tracking-wider border border-indigo-100 dark:border-indigo-900/30 shadow-sm">
    ...
    return (
        <div className="p-8 lg:p-12 max-w-[1600px] mx-auto space-y-10">
            <div className="flex justify-between items-end animate-in fade-in slide-in-from-top-4 duration-700">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white font-display tracking-tight">{t('reports.title')}</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">{t('reports.subtitle')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-800 transition-all hover:shadow-xl hover:shadow-blue-500/5 dark:hover:shadow-blue-900/10 group">
                    ...
                    <div className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter mb-1">{totalOpen}</div>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-800 transition-all hover:shadow-xl hover:shadow-blue-500/5 dark:hover:shadow-blue-900/10 group">
                    ...
                    <div className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter mb-1">{totalClosed}</div>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-800 transition-all hover:shadow-xl hover:shadow-blue-500/5 dark:hover:shadow-blue-900/10 group">
                    ...
                    <div className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter mb-1">{openActions.length}</div>
                    <div className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.15em]">{t('reports.pendingActions')}</div>
                </div>
            </div>

            {/* Actions Table */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-800 overflow-hidden animate-in fade-in duration-1000 delay-300">
                <div className="px-8 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                    <h3 className="font-black text-slate-700 dark:text-slate-300 text-xs uppercase tracking-[0.2em] flex items-center gap-3">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 border-separate border-spacing-0">
                        <thead className="bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 font-black border-b border-slate-100 dark:border-slate-800">
                            <tr>
                                <SortHeader label={t('table.dueDate')} sortKey="date" currentSort={sortConfig} onSort={handleSort} className="px-8 py-5 text-[10px] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800" />
                                <SortHeader label={t('table.actions')} sortKey="action" currentSort={sortConfig} onSort={handleSort} className="px-8 py-5 text-[10px] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800" />
                                <SortHeader label={t('table.responsible')} sortKey="responsible" currentSort={sortConfig} onSort={handleSort} className="px-8 py-5 text-[10px] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800" />
                                <SortHeader label={t('common.box')} sortKey="status" currentSort={sortConfig} onSort={handleSort} className="px-8 py-5 text-[10px] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800" />
                            </tr>
                        </thead>
                            ...
                            {openActions.map((action, idx) => {
                                ...
                                return (
                                    <tr key={`${action.rca_id}-${idx}`} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all">
                                        ...
                                        <td className="px-8 py-6 max-w-xs truncate font-bold text-slate-800 dark:text-slate-100" title={action.action}>{action.action}</td>
