import { isStepVisible } from '@/services/TemplateService';
...
export const getWizardSteps = (t: (key: string) => string, analysisTypeId?: string, showHra?: boolean): WizardStep[] => {
    const allSteps: WizardStep[] = [
        ...
        {
            id: 9,
            title: t('wizard.stepNames.step9.title') || 'HRA',
            subtitle: t('wizard.stepNames.step9.subtitle') || 'Análise de Confiabilidade Humana',
            fields: ['human_reliability']
        }
    ];

    // Filter by visibility based on template and HRA trigger
    return allSteps.filter(step => {
        if (step.id === 9) return showHra && isStepVisible(analysisTypeId, 9);
        return isStepVisible(analysisTypeId, step.id);
    });
};
