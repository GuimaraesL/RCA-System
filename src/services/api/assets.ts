...
export const importAssetsToApi = async (assets: any[]): Promise<void> => {
    ...
    if (isAlreadyFlat) {
        console.log('API: Lista plana detectada. Importando sem transformações.');
        flat = assets.map(a => ({
            id: a.id,
            name: a.name,
            type: a.type,
        ...
        console.log('API: Árvore de ativos detectada. Realizando achatamento (flattening)...');
    ...
    try {
        ...
        console.warn('API: Falha ao limpar alguns ativos, prosseguindo com a importação...', e);
    }
