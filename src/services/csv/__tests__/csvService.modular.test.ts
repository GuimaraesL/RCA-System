/**
 * Teste: csvService.modular.test.ts
 * 
 * Proposta: Validar a lógica de importação e exportação de dados em formato CSV utilizando a nova estrutura modularizada.
...
import { importFromCsv, exportToCsv, getCsvTemplate } from '../index';
import { AssetNode, TaxonomyConfig, RcaRecord } from '../../../types';

describe('csvService Modularizado', () => {
    ...
    describe('Exportação de RESUMO DE REGISTROS', () => {
        ...
        it('deve exportar e importar corretamente o modo árvore (chains) dos 5 Porquês', () => {
            const chains = [{
                chain_id: 'C1',
                cause_effect: 'Efeito 1',
                root_node: { id: 'N1', level: 0, whys: [], children: [] }
            }];
            const records: RcaRecord[] = [{
                id: 'R1',
                five_whys_chains: chains,
                status: 'S1',
                failure_date: '2023-01-01'
            } as any];

            const csv = exportToCsv('RECORDS_SUMMARY', { records });
            expect(csv).toContain('five_whys_chains');
            expect(csv).toContain('Efeito 1');

            const result = importFromCsv('RECORDS_SUMMARY', csv, { records: [] });
            expect(result.success).toBe(true);
            expect(result.data[0].five_whys_chains).toBeDefined();
            expect(result.data[0].five_whys_chains[0].chain_id).toBe('C1');
        });
    });
